...
class Lzx {
    ...
    decompress(buffer, frame_size, block_size) {
        ...
        if (!this.header_read) {
            ...
            Debug(`Intel: 0b${intel.toString(2)} = ${intel}`);
        ...
        while (togo > 0) {
            ...
            if (this.block_remaining == 0) {
                ...
                Debug(`Blocktype: 0b${this.block_type.toString(2).padStart(3,'0')} = ${this.block_type}`);
                ...
                Debug(`Block Remaining: ${this.block_remaining}`);
            ...
            for (let symbol = 0; symbol < symbols; symbol++) {
                ...
                if (length[symbol] == bit_num) {
                    ...
                    if ((pos += bit_mask) > table_mask) {
                        Debug(length[symbol]);
                        Debug(`pos: ${pos}, bit_mask: ${bit_mask}, table_mask: ${table_mask}`);
                        Debug(`bit_num: ${bit_num}, bits: ${bits}`);
                        Debug(`symbol: ${symbol}, symbols: ${symbols}`);
